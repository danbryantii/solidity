...
.. warning::
    ...
    Yul is in active development and bytecode generation is only fully implemented for untyped Yul (everything is ``u256``)
...
Yul parses comments, literals and identifiers in the same way as Solidity, so you can e.g. use ``//`` and ``/* */`` to signify comments.
usual ``//`` and ``/* */`` comments. There is one exception:
Identifiers in Yul can contain dots: ``.``.
...
Multiple syntactical elements can follow each other simply seperated by whitespce,
...
Depending on the optimiser settings, the compiler will/can free the stack slot already after...
